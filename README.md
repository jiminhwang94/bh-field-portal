...
- 가이드·재고·리포트·항목 변경은 **인터넷이 되는 순간 자동으로** 구글 시트에 올라가 팀 전체에 적용됨
  (오프라인이면 쌓였다가 연결되면 올라감). 상단 **[새로고침]** 은 다른 사람이 바꾼 것을 지금 받아오는 버튼
...
**내비게이션**: 좌측 레일(좁은 화면은 하단 탭바) — 홈 · 재고 · 새 리포트 · 이력, 맨 아래 설정.
상단바에는 동기화 칩과 [새로고침] 만 둔다. `항목 설정` 은 매일 쓰는 화면이 아니라 설정 안에 있다.
...
- 수량 `[-]/[+]` 는 시트의 해당 칸에 **즉시 기록**됩니다 — LTE 에서도, 어느 와이파이에서도 됩니다.
...
- **매장 → 날짜 → 사진/동영상** 순입니다. 매장을 먼저 찾고 그 날 무엇이
  있었는지 보는 순서라, 드라이브에서 직접 뒤질 때 이 순서가 가장 빠릅니다.
...
**올리기는 자동입니다.** 재고 수량 · 리포트 · 가이드 · 항목 설정을 바꾸면 인터넷이 되는
순간 곧바로 구글 시트로 올라갑니다. 오프라인이면 대기열에 쌓였다가 연결되면 올라갑니다.
따로 누를 것이 없습니다.

상단 오른쪽 **[새로고침]** 은 **다른 사람이 바꾼 내용을 지금 받아오는** 버튼입니다.
앱으로 돌아올 때와 5분마다 자동으로도 받아오므로, "지금 당장" 보고 싶을 때만 누르면 됩니다.

### 여러 사람이 동시에 써도 섞이지 않는 이유
가이드 · 항목 설정 · 차량/품목 구성은 시트 탭을 **통째로 다시 쓰는** 종류입니다.
그래서 올릴 때 먼저 시트 것을 받아 **내가 바꾼 줄만 얹고** 나머지는 시트 것을 그대로 둡니다.
A 가 1번 가이드를, B 가 2번을 고쳐 각자 올려도 둘 다 남습니다.
(예전에는 마지막에 올린 사람의 기기 내용이 다른 사람 변경을 통째로 덮어썼습니다)

### 상단바 칩 — 밀린 것이 있을 때만 숫자를 보인다
| 칩 표시 | 뜻 |
|---|---|
| `시트와 같은 내용 · 09-01 11:55` | 정상. 마지막으로 받아온 시각 |
| `올리는 중 3건` (파란색) | 곧 자동으로 올라감 |
| `오프라인 · 올릴 것 3건` (주황색) | 인터넷이 없음. 연결되면 올라감 |
| `올리지 못한 것 3건` (빨간색) | 올리기가 실패함. 칩을 눌러 이유를 본다 |

칩을 누르면 **[올릴 내용]** 목록이 열립니다. 항목을 누르면 고칠 화면으로 가고,
[되돌리기] 를 누르면 그 변경을 **바꾸기 전으로** 되돌립니다. 오프라인에서 **새로 만든**
리포트·가이드·품목을 취소하면 그 자료도 함께 지워집니다 (올라간 적이 없으니 남길 이유가 없습니다).

> v3.1 까지는 [⬆ 업데이트] 를 눌러야 팀에 적용됐고, 사무실 서버가 그 중심이었습니다.
> 팀 공유의 원본이 구글 시트 하나로 옮겨간 뒤 서버는 하는 일이 없어졌고, 올리기는 자동이 되어
> 버튼은 "받기" 하나만 남았습니다 (v3.9).
...
| 다른 사람 변경 받기 ([새로고침]) | ❌ (연결되면 자동) | ✅ 자동 + 버튼 |
...
### 처음 한 번만 필요한 것
`⚙️ 설정 → 구글 시트 연결` 에 웹 앱 URL 을 넣고 인터넷이 되는 곳에서 한 번 [새로고침] 하면
가이드·재고·항목이 내려옵니다. 리포트 항목은 앱에 붙박이로 들어 있어 시트 연결 전에도 쓸 수 있습니다.
...
> 사내망 `http://192.168.x.x` 로 접속하면 **자료는 기기에 남지만 앱 화면을 새로 여는 데는
> 화면 서버가 필요합니다.** 이 제약이 없는 쪽이 APK 설치입니다.
...
**③ 첫 실행 (한 번만)**
1. `⚙️ 설정 → 구글 시트 연결` 에 Apps Script 웹 앱 URL 입력
2. 상단 [새로고침] — 가이드·재고·항목이 내려옵니다
3. 끝. 이제 인터넷 없이도 전부 동작하고, 인터넷이 되면 자동으로 올라갑니다.

### 6-2. 새 버전 배포
화면·기능이 바뀌면 위 ①~② 를 다시 하면 됩니다(기존 앱에 덮어 설치).
**자료는 구글 시트에서 자동으로 받아오므로**, 자료만 바뀐 경우에는 새 APK 가 필요 없습니다.
...
| `media` | 사진 원본(blob) |
| `outbox` | 연결되면 시트로 올릴 작업 — **바꾸기 전 모습**을 함께 담아 되돌릴 수 있다 |
| `meta` | 설정 · 마지막 새로고침 시각 · 시트 사본 |

**서버** — 자료를 갖지 않습니다. `server.py` 는 `web/` 의 화면 파일을 내주는 것이 전부이고,
APK 로 설치한 앱은 서버 없이 동작합니다. 팀이 함께 보는 원본은 **구글 스프레드시트**입니다.

| 시트 탭 | 내용 |
|---|---|
| `YYYY-MM` (항목이 바뀌면 `YYYY-MM (2)` …) | 현장 리포트 — 한 달에 한 탭 |
| `차량재고` | 차량 · 품목 · 수량 · 최소보유 |
| `오류 코드 가이드` · `하드웨어 교체 SOP` · `SW·명령어` | 가이드 3종 |
| `리포트 항목` | 리포트 입력 항목 (팀 공통, 시트 순서 = 열 순서) |

> 리포트는 기기에 있다가 구글 시트로 갑니다. 사진·영상은 구글 드라이브에 갑니다.
...
│       ├── sync.js            연결 상태 · 대기열을 시트로 올리기
│       ├── sheets.js          구글 시트 직접 전송
│       ├── net.js             오프라인 띠 · 첫 실행 준비
│       ├── ui.js              토스트 · 모달 · 클립보드
│       ├── syncnow.js         [새로고침] · 상단 칩 · 자동 받기
│       ├── pending.js         [올릴 내용] 목록 · 되돌리기
│       ├── fieldsheet.js      리포트 항목 ↔ 시트 (팀 공통)
...
- **백업**: `tools\백업 자동실행 등록.bat` 을 한 번 더블클릭하면 **매일 저녁 7시**에
  ...
  ```
- **테스트**: `tests/` 8종. GitHub Actions 로 push 마다 자동 실행된다.
