...
`⚙️ 설정 → 구글 시트 연결 → [🚐 차량 재고를 구글 시트로 관리]` 를 켜면
스프레드시트의 **차량재고** 탭이 팀 공유 원본이 됩니다.
...
- 처음 켜면: 시트가 비어 있으면 기기 재고로 탭을 만들고, 이미 내용이 있으면 그쪽을 받아옵니다.
- 이 모드에서는 [⬆️ 업데이트]가 차량·재고·수량을 덮지 않습니다 (가이드·항목 설정만 서버 동기화).
- 최소보유는 부품당 한 칸이라 **모든 차량에 같은 값**이 적용됩니다.
- 모든 기기에서 같이 켜 주세요. 스크립트를 재배포해야 동작합니다(아래 참고).
...
*방법 B. USB 로* (태블릿에 인터넷이 없어도 됨)
1. PC 에서 Releases 의 `.apk` 를 내려받아 **USB 에 복사**합니다.
